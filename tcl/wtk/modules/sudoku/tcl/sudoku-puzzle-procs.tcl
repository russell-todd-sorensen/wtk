...
<< ::Sudoku::Puzzle.method init {} {
    ...
    ::wtk::log::log Debug "symbol_index = $symbol_index"
    ::wtk::log::log Debug "symbols = $symbols"
    for {set i 0} {$i < $puzzleDimension} {incr i} {
        set index  [scan [lindex $symbols $i] %c]
        ::wtk::log::log Debug " i = $i, index = $index"
    ...
    ::wtk::log::log Debug "activeCells = $activeCells"
    ...
    for {set i 1} {$i <= $activeCells} {incr i} {
        ...
        if {[lindex $puzzleData $i] == "0"} {
            lappend cellType "A"
            ::wtk::log::log Debug "i = $i, Active cell"
            ...
            ::wtk::log::log Debug "Puzzle.init: i = '$i', \[lindex $puzzleData $i\] = '[lindex $puzzleData $i]'"
            set cellValue [lindex $symbol_index $index]   ;# ???
            ::wtk::log::log Debug "Puzzle.init: index = '$index', cellValue = '$cellValue'"
            set CR [lindex $cellRow $i]
            set CC [lindex $cellCol $i]
            set CB [lindex $cellBox $i]
            ::wtk::log::log Debug "i = $i, CR = $CR, CC = $CC CB = $CB cellValue = $cellValue"
            #::wtk::log::log Debug "\[lindex \$rowMask $CR\] & ~(1<< $cellValue) = [lindex $rowMask $CR] & [expr {~(1<<$cellValue)}] = [expr {[lindex $rowMask $CR] & ~(1<< $cellValue)}]"
            ...
            ::wtk::log::log Debug "rowMask $i = [lindex $rowMask $i]"
            ::wtk::log::log Debug "colMask $i = [lindex $colMask $i]"
            ::wtk::log::log Debug "boxMask $i = [lindex $boxMask $i]"
